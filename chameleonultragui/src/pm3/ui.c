...
// #define DEBUG_LOG_TO_FILE "/tmp/recovery_log"
...
void PrintAndLogEx(logLevel_t level, const char *fmt, ...)
{
    ...
    case ERR:
        if (g_session.emoji_mode == EMO_EMOJI)
            strncpy(prefix, "[" _RED_("!!") "] :rotating_light: ",
                    sizeof(prefix) - 1);
        else
    ...
    case FAILED:
        ...
        break;
    case DEBUG:
        strncpy(prefix, "[" _BLUE_("#") "] ",
                sizeof(prefix) - 1);
        break;
    case HINT:
        strncpy(prefix, "[" _YELLOW_("?") "] ",
                sizeof(prefix) - 1);
        break;
    case SUCCESS:
        strncpy(prefix, "[" _GREEN_("+") "] ",
                sizeof(prefix) - 1);
        break;
    case WARNING:
        if (g_session.emoji_mode == EMO_EMOJI)
            strncpy(prefix, "[" _CYAN_("!") "] :warning:  ",
                    sizeof(prefix) - 1);
        else
    ...
    case INPLACE:
        if (g_session.emoji_mode == EMO_EMOJI)
        {
            strncpy(prefix, spinner_emoji[PrintAndLogEx_spinidx],
                    sizeof(prefix) - 1);
        ...
        {
            strncpy(prefix, spinner[PrintAndLogEx_spinidx],
                    sizeof(prefix) - 1);
            PrintAndLogEx_spinidx++;
    ...
    // For NORMAL level, no prefix processing is done.
    if (level == NORMAL)
    {
        fPrintAndLog(stream, "%s", buffer);

#ifdef DEBUG_LOG_TO_FILE
        FILE *recovery_fp = fopen(DEBUG_LOG_TO_FILE, "a");
        if (recovery_fp)
        {
    ...
    else
    {
        ...
        if (level == INPLACE)
        {
            ...
            memcpy_filter_ansi(buffer3, buffer2, sizeof(buffer2),
                               !g_session.supports_colors);
            memcpy_filter_emoji(buffer4, buffer3, sizeof(buffer3),
                                g_session.emoji_mode);
        ...
        for (size_t i = 0; i < n; i++)
        {
            ...
            { // entering ANSI sequence
                ...
                { // entering CSI sequence
                    ...
                    while ((i < n - 1) && (rsrc[i] >= 0x30) && (rsrc[i] <= 0x3F))
                    { // parameter bytes
                        i++;
                    }

                    while ((i < n - 1) && (rsrc[i] >= 0x20) && (rsrc[i] <= 0x2F))
                    { // intermediate bytes
                        i++;
                    }

                    if ((rsrc[i] >= 0x40) && (rsrc[i] <= 0x7F))
                    { // final byte
                        continue;
                    }
                }
